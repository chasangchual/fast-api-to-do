...
from app.setvices.auth_bearer_service import JWTBearer
...
@auth_router.post("/hello", status_code=status.HTTP_201_CREATED)
@inject
async def signup(request: NewUserRequest, session: db_session,
                 ...
                 user: User = Depends(JWTBearer())) -> UserResponse:
    try:
